...
for i in closeprice.index:
    ...
    for j in closeprice.columns:
        ...
        if int(hat.loc[i,j])==1:
            ...
            if float(closeprice.iloc[i,j])*1.1<=float(closeprice.loc[row+1,col]):
                out.loc[i,j]=1
            elif float(closeprice.iloc[i,j])*0.9>=float(closeprice.iloc[row+1,col]):
                out.loc[i,j]=-1
            else:
                out.loc[i,j]=0
        col+=1
